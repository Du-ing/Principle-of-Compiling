...
//读取接口数据
void read_before(ifstream& inFile){
    string line;
...
int p3(){
    ...
    for(int i = 0; i < st.accept.size(); i++){
        outFile << st.accept.at(i);
        //cout << st.accept.at(i);
        if(i < st.accept.size() - 1)
            outFile << " ";
    ...
    for(int i = 0; i < result_set.size(); i++){
        ...
        for(int j = 0; j < state.accept.size(); j++){
            cout << state.name << "-" << state.accept.at(j) << "->" << state.change.at(j) << " ";
            outFile << state.name << "-" << state.accept.at(j) << "->" << state.change.at(j) << " ";
        }
        cout << endl;
        outFile << endl;
    }

...
//打印某一个节点的状态转移规则
void print_statuslist(vector<Edge>& statuslist,Node first,Node end,ofstream& outFile)
{
    for(auto edge:statuslist)
    {
        ...
        outFile<<" "<<edge.startNode<<"-"<<edge.character<<"->"<<edge.endNode; 
    }
    cout<<endl;
    outFile<<endl;
}
...
{
    ofstream outFile;
    outFile.open("outFile2.txt",ios::app);
    int cnt=0;
    ...
    outFile<<"X";
    cout<<"X";
    print_statuslist(iter_begin->second,first,end,outFile);
    cout<<"Y";
    outFile<<"Y";
    print_statuslist(iter_end->second,first,end,outFile);
    for(auto iter = ++iter_begin; iter != iter_end; iter++)  //从初态X的下一个开始遍历
    {
        cout<<iter->first-'1';
        outFile<<iter->first-'1';
        print_statuslist(iter->second,first,end,outFile);
    }
    outFile.close();
}
...
int p2()
{
    ...
    string filename="outFile1.txt";//用于测试，以文件方式读入
